...
function fetchTradesByUserQuery (userId) {
  return Trade.aggregate([
    { "$facet": {
      "trades": [
        { "$match": {"tradeCreator": userId}},
        { "$limit": 5 },
        { "$sort": {"createdAt": -1} }
      ],
      "tradesCount": [
  ...
  .then(results => {
    return new Promise((resolve, reject) => {
      Category.populate(results[0].trades, {path: 'category'})
      .then(pTrades => {
        if (pTrades && pTrades.length > 0) {
          resolve({data: pTrades, count: results[0].tradesCount[0].count});
        } else {
          resolve({data: results[0].trades, count: 0})
        }
      })
    })
  })
