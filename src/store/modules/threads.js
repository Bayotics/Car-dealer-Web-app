...
export default {
  ...
  actions: {
    fetchThreads ({state, commit}, {tradeId, filter = {}, init}) {
      if (init) {
        commit('setItems', {resource: 'threads', items: []}, {root: true})
      }

      const url = applyFilters(`/api/v1/threads?tradeId=${tradeId}`, filter)
    ...
    postThread ({commit, state}, {title, tradeId}) {
      const thread = {}
      thread.title = title
      thread.trade = tradeId
